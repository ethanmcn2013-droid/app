...
FINDING id=partial-note-explains-the-design-to-the-owner
Seat: ux
Element: .note paragraph under the KPI row in ?state=partial
Problem: The note reads: "Timeline did not answer this reading, so nothing here knows a due date. The card keeps its place and says so; it does not fall back to zero." The first sentence is the owner's. The second is the studio's — it names a component ("the card"), describes a layout decision ("keeps its place") and defends a policy to a reviewer ("it does not fall back to zero"). r1 closed refusals-written-in-the-studio-vocabulary on the three limit tiles; the class is back, in a different element, and the paid defect library records it as raised three separate times in one previous engagement.
Consequence claimed: A venue owner is handed a sentence about the product's internal policy at the moment they wanted to know whether anything is overdue. It costs a line of reading and tells them nothing they can act on, and it is the only place on this surface where the product talks about itself.
Severity claimed: refinement
Proposed fix: Cut the second sentence and, if the note needs a second beat, replace it with the owner's next fact: "Timeline did not answer this reading, so nothing here knows a due date. Everything else on this page was read normally."
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-3\refuters\partial-note-explains-the-design-to-the-owner.json.
