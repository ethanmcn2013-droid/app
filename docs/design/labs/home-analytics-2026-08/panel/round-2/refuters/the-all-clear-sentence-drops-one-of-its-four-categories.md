...
FINDING id=the-all-clear-sentence-drops-one-of-its-four-categories
Seat: UX and information design
Element: quiet state, the "Nothing is sitting" card's body sentence
Problem: The full state's band counts four conditions - hasn't moved in a fortnight, past the day it was due, nobody's name on it, waiting on something else. The quiet state replaces all four cards with one sentence, and the sentence names three: "All 9 open jobs have moved inside a fortnight, none is past its day, and every one has a name on it." The fourth, waiting on something else, is zero in the quiet scope and is simply not mentioned. The band's own principle is stated in the code beside it - the band does not vanish, because an absent band reads as a bug - and the same reasoning applies one level down, to a category that vanishes from the sentence that replaces it.
Consequence claimed: An owner who has learned the four kinds of slippage from the full state reads the all-clear and finds only three of them cleared. The most natural reading of a missing item in a list of four is that it was not checked, which is the one impression this surface works hardest not to give.
Severity claimed: refinement
Proposed fix: Extend the sentence to enumerate all four in the row's order: "All 9 open jobs have moved inside a fortnight, none is past its day, every one has a name on it, and none is waiting on anything else." Keep the existing singular branch for D.open === 1.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-2\refuters\the-all-clear-sentence-drops-one-of-its-four-categories.json.
