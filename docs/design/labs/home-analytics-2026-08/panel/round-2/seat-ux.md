...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-2\seat-ux.json.

ENVIRONMENT NOTE (this machine)
`@playwright/test` resolves only from `C:\Users\ethan\signal-studio-workspace\collateral`.
Run every driving script with that as the working directory, and write throwaway
scripts into the scratchpad, never into the lab:
`C:\Users\ethan\AppData\Local\Temp\claude\C--Users-ethan\e00337f1-697d-4966-8b90-214de10e5345\scratchpad`

Chromium is installed. The master serves `?state=`, `?v=light|dark` and
`?motion=play|settled`; automation lands on `settled` by default, so pass
`?motion=play` when you want to watch the entrance. Grade the dark ground too.
