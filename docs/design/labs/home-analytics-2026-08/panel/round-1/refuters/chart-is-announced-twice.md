...
FINDING id=chart-is-announced-twice
Seat: Measured evidence
Element: .gridline b, .record-tag, .bar-val and .xaxis — none is aria-hidden; .xaxis span.mute uses opacity: 0
Problem: The chart carries a written equivalent in `.sr` ('Twelve weeks of finished work … 27 Apr, 5; 4 May, 3; … 13 Jul, 12 so far. The best week before this one was 8.'), and every piece of visual furniture is left in the accessibility tree beside it. Walking the chart's non-aria-hidden leaves returns, in order: '6', '12', 'Best week · 8', '12 so far', then all twelve x-axis dates — of which eight compute `opacity: 0`, because `.xaxis span.mute` hides them visually with opacity rather than visibility, which removes them from sight but not from the tree.
Consequence claimed: A screen-reader user hears the twelve weeks as a sentence and then hears the same chart again as a stream of loose numbers and dates, including eight dates that are not on the screen at all. The reading the sr line was written to give is buried in the duplicate.
Severity claimed: defect
Proposed fix: Hide the visual layer from assistive technology: add `aria-hidden="true"` to each `<span class="gridline">`, to `.record-tag`, to `.bar-val` and to the `<div class="xaxis">` wrapper; and change `.xaxis span.mute { opacity: 0 }` to `.xaxis span.mute { visibility: hidden }` so nothing invisible carries text at all.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\chart-is-announced-twice.json.
