...
FINDING id=two-capitalisation-rules-in-one-tab-strip
Seat: Brand and copy
Element: The tab strip - "Today's Signal", "Full briefing", "Lately"
Problem: Two sibling destinations, both proper names of surfaces in this product, are capitalised by two different rules on the same line. The signed-in naming contract names it the Full Briefing.
Consequence claimed: One of the three destinations is demoted from a name to a description, in every state and both grounds, on the most-read line of the page. It is the kind of inconsistency the benchmark products never ship, and once seen it is visible every time.
Severity claimed: refinement
Proposed fix: Render the tab "Full Briefing", matching Today's Signal and the naming contract.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\two-capitalisation-rules-in-one-tab-strip.json.
