...
FINDING id=record-tag-never-paints
Seat: Typography
Element: .record-tag inside .record, movement one's twelve-week chart
Problem: The label "Best week · 8" is in the DOM at full opacity and never reaches the screen. `.record` is a 1px-tall box (`height: 0` plus `border-top`), and its `draw` keyframes end on `clip-path: inset(0 0 0 0)` with `animation-fill-mode: both`. That fill sits above the `[data-motion="settled"] .record { clip-path: none }` rule in the cascade, so the clip is always applied and always resolves to the 1px border box. The tag is positioned at `top: -18px`, entirely outside it, and is clipped away. Verified at 390, 768, 1280 and 1440, on both grounds, under `?motion=settled`, `?motion=play` after the 1.7s choreography, and the webdriver default: `document.elementFromPoint` at the tag's own centre returns `.colw`, never the tag. Overriding `.record { animation: none !important; clip-path: none !important }` makes "BEST WEEK · 8" appear immediately — and reveals a second problem underneath, that the tag then sits behind the hatched partial-week bar, which overprints "K ·".
Consequence claimed: The owner sees a dashed rule crossing the chart at two-thirds height with nothing naming it. A dashed reference line in a chart conventionally reads as a target or a threshold, so she is invited to conclude "I am above the line" without being told what the line is or that it is worth 8. The one comparison the movement exists to make — is this stretch better than my normal — is drawn and left unlabelled. The screen-reader line does say "The best week before this one was 8", so the spoken reading carries the fact and the visual one does not: the sighted reader gets strictly less.
Severity claimed: misleading
Proposed fix: Take the tag out of the clipped box. Move `.record-tag` from a child of `.record` to a sibling absolutely positioned inside `.plot` at `bottom: calc(var(--r) * 1%)` with its own `margin-bottom: 4px`, keeping `right: 0` and the `t-label` role. Then reserve the space it needs: add `padding-right: 96px` to `.plot`'s `.cols`, `.record` and `.gridline` rules so the twelfth column no longer runs under the right-hand labels, and give `.record-tag` `background: var(--paper); padding-left: var(--gap-hair)` as it already has. If the entrance is still wanted, animate the tag's own `opacity` rather than clipping its parent.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\record-tag-never-paints.json.
