...
FINDING id=error-state-stamps-a-successful-read
Seat: UX and information design
Element: ?state=error — the .coverage strip against the card's "Last good reading"
Problem: On the state that says "Tasks did not answer", the coverage strip still prints "Read 09:14, Thursday 16 July 2026" — the same instant the card above calls "Last good reading · 09:14, Thursday 16 July 2026". One moment carries two names with opposite meanings on one screen: the reading that failed, and the last one that worked.
Consequence claimed: The reader trying to work out how stale the screen is gets two labels for one timestamp and cannot tell whether anything was read at all. The page's own rule is one grammar per fact, and the error state is where an anchored time matters most.
Severity claimed: refinement
Proposed fix: In the error state, have the coverage strip label the attempt rather than the reading — "Tried 09:14, Thursday 16 July 2026" — and let the card's "Last good reading" carry a genuinely earlier instant from the fixture. Apply the same wording to the loading state, where "Read" is also premature.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\error-state-stamps-a-successful-read.json.
