...
FINDING id=label-voice-set-on-a-paragraph
Seat: Typography
Element: .coverage — the source-and-provenance strip at the foot of every state
Problem: The micro-label voice (11px Geist Mono, weight 400, `letter-spacing: 0.09em`, uppercase) has no upper bound on string length, and the longest string on the surface is set in it. The `.coverage` block carries 246–267 characters of `data-type="label"` depending on state, of which the lab clause alone is 134: "LAB WEEKLY COUNTS AND AGES ARE LAB-AUTHORED; HARBOUR HOUSE WEDDINGS, ORLA, THE READING DATE AND THE 9 ARE BOUND TO THE SHIPPING FIXTURE". At 390 it renders as a four-line, 133px-tall paragraph of tracked uppercase mono. Every other label on the surface is a stamp of 12–36 characters, which is what the voice is for; this one is a sentence with a semicolon and three clauses. Tracked uppercase mono destroys word shape, and word shape is what makes a sentence scannable — the two are directly opposed.
Consequence claimed: The line that discloses which figures are real and which are lab-authored — the surface's own honesty statement, and the thing a sceptical owner would look for — is the hardest text on the page to read. It is set the way a legal disclaimer is set, which is exactly the reading it will get: skipped.
Severity claimed: defect
Proposed fix: Split the strip's voice. Keep the four short source stamps (`Tasks answered`, `Timeline answered`, `Notes answered, for you`, `Read 09:14…`) in `t-label`, and move the lab clause to its own line below them in `t-small dim`: change the last `<span>` in `shell()`'s `.coverage` block to `<span class="t-small dim" data-type="small" style="flex:1 1 100%"><b>Lab</b> …</span>` and add `.coverage .t-small { text-transform: none; letter-spacing: var(--track-flat); max-width: 68ch; }`.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\label-voice-set-on-a-paragraph.json.
