...
FINDING id=hero-count-starts-from-a-number-that-means-nothing
Seat: Product taste and emotional resonance
Element: the count-up on #count in the hero figure
Problem: The one animated numeral on the page counts from `target - 12` — 19 on this fixture — to 31 over 700ms. Nineteen is not a fact. The number it should count from is on screen two centimetres away: the delta pill says "+5 on the four before", so the four before is 26, and 26 is a real figure the reader can be shown earning its way to 31.
Consequence claimed: The page's only piece of emotional choreography spends itself on an arbitrary number. Counting 19 → 31 is decoration; counting 26 → 31 is the argument the page is making, performed. As it stands the entrance is pleasant and forgettable, which is the honest summary of the whole surface: nothing here is a moment you would remember or show anyone.
Severity claimed: refinement
Proposed fix: Set `const from = D.lastFour - D.delta;` in place of `Math.max(0, target - 12)`, clamped at 0, and hold the delta pill at opacity 0 until the count lands so the "+5" arrives as the conclusion of the count rather than as a caption printed before it. When the delta is negative the count runs down, which is equally true and equally worth seeing.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\hero-count-starts-from-a-number-that-means-nothing.json.
