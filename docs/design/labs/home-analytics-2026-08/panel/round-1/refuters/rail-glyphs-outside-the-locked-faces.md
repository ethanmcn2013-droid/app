...
FINDING id=rail-glyphs-outside-the-locked-faces
Seat: Typography
Element: .rail i — the four navigation glyphs at top left
Problem: The rail draws U+2302 ⌂, U+270E ✎, U+2611 ☑ and U+2197 ↗ with `font-family: Geist`. Geist does not carry the first three. Probed with `canvas.measureText` at 13px: ⌂ measures 7.8px under Geist, Geist Mono, serif and system-ui alike, ✎ measures 13px under all of them and ☑ 11.2px under all of them — identical widths meaning one fallback font is painting them in every case — while the control glyph "A" measures 8.68 under Geist against 9.39 serif and 8.39 system-ui, and ↗ measures 8.07 under Geist against 9.52 elsewhere, so Geist really does carry that one. Three of the four rail marks are therefore drawn by whatever the OS supplies. The audit's family check cannot see it: it reads computed `font-family`, which reports what the author wrote ("Geist"), not what the compositor painted.
Consequence claimed: The first four objects at the top left of every frame at 768 and above are the only shapes on the surface not drawn by a face the studio chose, and they change between Windows, macOS and Linux — so the same screenshot is not the same product. On systems that resolve U+2611 to an emoji face it would also paint in colour, outside the eighteen.
Severity claimed: refinement
Proposed fix: Replace the four glyph characters in `shell()`'s `.rail` markup with inline SVG paths at 13px in `currentColor`, drawn on the same 12×12 grid as the four status marks in `ICON`, so the rail is vector like the rest of the surface and no text is set in a face outside the two.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\rail-glyphs-outside-the-locked-faces.json.
