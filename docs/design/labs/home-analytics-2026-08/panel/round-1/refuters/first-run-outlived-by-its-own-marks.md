...
FINDING id=first-run-outlived-by-its-own-marks
Seat: Brand and copy
Element: State first-run - the hero "12 jobs finished since you started", the header's "Twelve weeks", and the ages strip's "Oldest . 41 days"
Problem: The state exists for a workspace with fewer than four weeks of records: it draws one column, and says the comparison arrives on 10 August. The same screen then states that three jobs have not moved in a fortnight and that the oldest open job has been sitting 41 days - six weeks, which would mean four weeks of records already exist and the comparison is overdue. Above both, the header's window label still reads "Twelve weeks" over a chart with one column; it reads the same on empty and error, where nothing is drawn at all.
Consequence claimed: A new owner - the reader with the least reason to trust this yet - is given two incompatible accounts of how long she has been using the product, on the one screen whose job is to prove the product is careful. One of the two numbers must be wrong and she has no way to tell which.
Severity claimed: defect
Proposed fix: Give first-run its own job ages, none greater than the days between the first recorded week and the reading instant, so the strip cannot outrun the record it sits under. Make the header's window label read the window actually drawn: "One week" in first-run, "Twelve weeks" where twelve are drawn, and no label at all in empty and error, where none are.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\first-run-outlived-by-its-own-marks.json.
