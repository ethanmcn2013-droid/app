...
FINDING id=live-value-collides-with-axis-label
Seat: Interaction and states
Element: .bar-val ("12 so far") against .gridline b ("12") in the twelve-week chart
Problem: The gridline values are anchored to the plot's right edge (.gridline b { right: 0 }) and the live column is the last column, so its value label is anchored to the same edge. They overlap by 154 square pixels at 1440, 1280, 900, 768, 560 and 390 — every width measured — printing "12 SO FAR" through the axis's own "12". The label also overhangs the plot box to the right, by 7px at 1440 rising to 23px at 390, where it breaks the card's right edge. It is visible in the delivered light-full frames at both 1440x960 and 390x844.
Consequence claimed: The one number the chart prints — the figure carrying the whole "so far" reading of the unfinished week, and the only indigo type on the screen — is rendered on top of another number. At a glance it reads as a rendering fault, which is the last impression an instrument arguing for its own honesty can afford.
Severity claimed: defect
Proposed fix: Move the axis values to the left edge, where no column label competes for the space: .gridline b { right: auto; left: 0; padding-left: 0; padding-right: var(--gap-hair); }. Then keep the live column's value inside the plot by anchoring it to that column's right edge instead of its centre: .cols .colw:last-child .bar-val { left: auto; right: 0; transform: none; }.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\live-value-collides-with-axis-label.json.
