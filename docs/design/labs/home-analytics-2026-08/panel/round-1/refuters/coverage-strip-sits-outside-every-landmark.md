...
FINDING id=coverage-strip-sits-outside-every-landmark
Seat: Measured evidence
Element: div.coverage, a sibling of <main> inside .page
Problem: `.page` contains `<header class="top">`, `<main>` and then `<div class="coverage">`. The coverage strip — which sources answered, which numbers the lab authored, when the reading was taken — belongs to no landmark, so it is skipped by landmark navigation and is not part of 'read the main content'.
Consequence claimed: The surface's provenance apparatus, which is the thing that makes its honesty checkable, is the one region a screen-reader user is least likely to land on.
Severity claimed: refinement
Proposed fix: Wrap it: change `<div class="coverage t-label" data-type="label">` to `<footer class="coverage t-label" data-type="label" aria-label="What this reading was built from">` and close it as `</footer>`. No CSS changes — the class carries the layout.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\coverage-strip-sits-outside-every-landmark.json.
