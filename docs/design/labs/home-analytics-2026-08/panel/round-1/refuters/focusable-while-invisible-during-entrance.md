...
FINDING id=focusable-while-invisible-during-entrance
Seat: Interaction and states
Element: a.kpi.rise and .dot::before, at ?motion=play
Problem: The entrance animates opacity from 0 with staggered delays (rise up to 260ms delay, .dot::before up to 760ms + 55ms per mark) while every control is focusable from the first frame. Measured at ?motion=play: 1203ms elapse before no focusable element is at effective opacity below 0.1 — 12 invisible focusables at ready, still 3 at 900ms. Four Tab presses immediately after __LATELY_READY land on the "3 haven't moved in a fortnight" KPI link at effective opacity 0.000; its 2px focus outline inherits that opacity, so nothing at all is painted. The behaviour gate's "nothing invisible can take focus" rule runs on the settled surface (default under webdriver), so this condition has never been evaluated by it — the same blind-gate class as the coarse-pointer and sub-720px misses in the defect library.
Consequence claimed: A keyboard user who starts tabbing on arrival — the fastest users, and the ones for whom the ring is the only wayfinding — loses the ring entirely for over a second. There is no way to tell whether focus has entered the page, stalled, or left it, and the next Tab is a guess.
Severity claimed: defect
Proposed fix: Let the first keyboard engagement end the choreography. Add, alongside the existing keydown handler: document.addEventListener('keydown', (e) => { if (e.key === 'Tab') document.documentElement.setAttribute('data-motion', 'settled'); }, { capture: true, once: true }); and the same on the first focusin inside #root. The existing [data-motion="settled"] block already snaps every animation to 1ms with zero delay and restores the settled transforms, so the surface is complete before focus lands on the first control.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\focusable-while-invisible-during-entrance.json.
