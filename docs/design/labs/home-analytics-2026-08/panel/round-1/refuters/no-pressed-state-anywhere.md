...
FINDING id=no-pressed-state-anywhere
Seat: Interaction and states
Element: a.kpi, .dot, .tab, .btn
Problem: The file contains no :active rule at all. Measured with the primary button actually held down: a.kpi, .dot and .tab all compute identically under hover and under press (transform, background, border, colour, ::before background all unchanged), and .btn's press is byte-identical to its hover. Separately, .dot — the surface's only genuinely interactive object — computes cursor: default, while a.kpi, .tab and .btn all compute pointer, so the nine marks give the pointer less acknowledgement than the cards that go nowhere.
Consequence claimed: Under a coarse pointer there is no hover, so the press is the only channel by which the surface can confirm a touch registered — and it says nothing. Tapping a mark and tapping 2px beside it feel identical, which is exactly the ambiguity that makes a person tap a third and fourth time. On the desk, the marks never invite the pointer in the first place, so the reading that lives only in the tooltip stays undiscovered.
Severity claimed: defect
Proposed fix: Declare a pressed state for each control after the hover and focus rules, using tokens already in the lock: a.kpi:active { transform: translateY(0); border-color: var(--ink-4); } · .btn:active { background: var(--paper-3); border-color: var(--ink-3); transform: translateY(1px); } · .tab:active { color: var(--ink-2); } · .dot:active::before { transform: scale(0.82); } (on the pseudo, so the hit area is untouched). Add cursor: pointer to the .dot rule.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\no-pressed-state-anywhere.json.
