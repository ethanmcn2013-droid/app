...
FINDING id=count-templated-onto-a-fixed-verb
Seat: Typography
Element: the quiet state's clear-all sentence, and the accessible name built in kpi()
Problem: Two strings concatenate a number with a verb phrase that was written for one count only. On screen in `state=quiet`: "All 9 open jobs **has** moved inside a fortnight, none is past its day, and every one has a name on it." — `plural(D.open, "open job", "open jobs")` produces a plural subject and the verb is hard-coded singular. In the accessibility tree in every state that renders the unowned card: "1 job **have** nobody's name on them, of 9 jobs open" — `kpi()` builds `${plural(n, "job", "jobs")} ${word}` where `word` is a fixed plural phrase.
Consequence claimed: The quiet state is the one moment the product tells the owner nothing is wrong, and it says it in a broken sentence — the surface reads as assembled by a machine at precisely the point it is asking to be believed. The screen-reader reading of the unowned card is ungrammatical every time it is spoken, on every state, which is the one channel a reader cannot skim past.
Severity claimed: defect
Proposed fix: In the quiet view, replace `All ${plural(D.open, "open job", "open jobs")} has moved` with `Every one of the ${plural(D.open, "open job", "open jobs")} has moved`. In `kpi()`, change the accessible-name template from `` `${plural(n, "job", "jobs")} ${word}, of …` `` to `` `${n} of ${plural(D.open, "open job", "open jobs")}: ${word}` ``, which agrees for every count including 1 and 0 without a second phrasing.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\count-templated-onto-a-fixed-verb.json.
