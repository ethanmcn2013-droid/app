...
FINDING id=kpi-row-and-ages-card-share-an-edge
Seat: UX and information design
Element: .kpis and the .card.ages beneath it inside movement two
Problem: There is zero space between them. Measured at 1440 in state=full, the ages card's top equals the KPI row's bottom exactly (gap 0), so the two cards' 1px hairlines stack into a doubled seam and read as one card that has cracked. The same is true in the quiet state, where the reassurance card and the ages card touch. movementTwo() concatenates `${strip()}` after .kpis with no separating token.
Consequence claimed: Two separate objects — a row of counts and a distribution over time — merge into one ambiguous shape, so the reader cannot tell where the claim ends and the evidence begins. It is also the only place on the page where the space ladder is not applied at all, which makes it read as an oversight rather than a decision.
Severity claimed: defect
Proposed fix: Wrap movement two's contents in a flex column with `gap: var(--gap-line)`, or add `margin-top: var(--gap-line)` to .ages, so the KPI row and the ages card are separated by a rung at every width. Apply the same separation in the quiet state between the reassurance card and the strip.
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\kpi-row-and-ages-card-share-an-edge.json.
