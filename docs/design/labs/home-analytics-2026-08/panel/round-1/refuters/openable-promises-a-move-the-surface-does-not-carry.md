...
FINDING id=openable-promises-a-move-the-surface-does-not-carry
Seat: Brand and copy
Element: Hero small print - "Counted at the moment each job reached a done column. Every one of them is openable."
Problem: Nothing on this surface opens a finished job. The numeral is not a control, the twelve columns carry no interaction at all, and the only links in the view are the three status cards, which are about open work. "Openable" is also not a word this audience uses, and it is the only coined word on an otherwise plain-English screen.
Consequence claimed: The reader takes the sentence as an invitation, looks for the way in, and finds none. An advertised affordance that does not exist costs more than saying nothing.
Severity claimed: refinement
Proposed fix: Replace with "Counted when each job reached a done column. Each one is a real job, and you can open it in Tasks."
...
Write the JSON to C:\Users\ethan\signal-studio-workspace\_wt-home-analytics\docs\design\labs\home-analytics-2026-08\panel\round-1\refuters\openable-promises-a-move-the-surface-does-not-carry.json.
