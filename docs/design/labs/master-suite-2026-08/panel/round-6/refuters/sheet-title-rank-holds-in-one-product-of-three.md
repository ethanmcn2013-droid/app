...
FINDING id=sheet-title-rank-holds-in-one-product-of-three
Seat: type
Element: h1.headName — src/tasks.css:287-292 (20px/600, --tr-19, ink-1) vs src/notes.css:287 (var(--t-15)/400, --tr-14, color: var(--ink-2)); the competing objects are span.word (17px/600, src/shell.css:535) and button.headNext b (15px/600, src/notes.css:285)
Problem: tasks.css:288-291 writes the suite's rule down in the file: 'The sheet's own title is the largest thing on it. Four sizes inside a 2px band, all at 600, gave the top of the scale no rank at all.' Measured in the 60-66px head band at 1920, one product of three keeps it. Tasks: h1 'The Orchard, events' at 20px/600 in ink-1 is the largest text on the sheet head, 3px above the wordmark — clean, one title, one rank. Notes: the same class on the same element is 15px/400 in ink-2, so the h1 'All your notes' is the THIRD-ranked object in its own head, out-ranked by span.word 'notes' at 17px/600 and tied on size but beaten on weight by the sibling button.headNext b 'Mara & Finn' at 15px/600. The largest word on the Notes sheet head is the product's own name; the largest word on the Tasks sheet head is the venue. Timeline puts its h1 (h1.b-who 'Mara & Finn') at 12px/400 Geist Mono uppercase — the smallest text in the product. So a person walking the spine gets three different answers to 'which word on this head tells me where I am', and in Notes the answer is a 15px grey label sitting between two darker, heavier neighbours, with a 100px gap and no size step to separate the h1 from the toggle beside it. That is the one thing a shared head has to get right, and the rule to get it right is already written in the repo.
Proposed fix: In Notes, put the rank on the object that actually names the sheet. Move the h1 from 'All your notes' onto the filing name (the b inside button.headNext at src/notes.css:285) and set that rule to `font-size: var(--t-20); font-weight: 600; letter-spacing: var(--tr-21); color: var(--ink-1); line-height: var(--lh-title);` — 20px/600/ink-1, matching tasks.css:292 exactly. Then demote src/notes.css:287 .headName to the scope-label tier it actually is: `font-size: var(--t-13); font-weight: 400; letter-spacing: var(--tr-13); color: var(--ink-3);` and drop its h1 tag to a span, so 'All your notes' reads as the breadcrumb it is rather than a heading nobody can find. Leave Timeline's 12px mono ceremonial line where it is — the 148px '79' is that sheet's title and Direction B is the founder's — but give timeline's sheet head the 20px/600 title slot too by promoting the .b-switch timeline name (src/timeline.css, 15px/600) to var(--size-lead)+3 / 600 / --track-head, so all three heads step 20 → 17 → 13 from title to wordmark to metadata.
