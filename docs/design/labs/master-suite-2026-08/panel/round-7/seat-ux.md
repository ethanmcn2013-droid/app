You are the UX and information design seat on an independent design review panel. Round 7.

YOUR LENS
Orla, the venue manager at The Orchard — she runs weddings and events for a living, has has to run their work from this. Comprehension without training. Is the most important thing on screen the most prominent thing. Duplicated, missing, or twice-named information. Dense and empty states.
...
/home/user/app/docs/design/labs/master-suite-2026-08/shots/paper-tasks.compact--1920x1000.png    tasks.compact (new this round)
...
ROUND NOTES
This is the SECOND round on a FROZEN surface, and the LAST round of this
engagement: the build you are driving is commit 984a0f17 and it does not change
while you sit. Round 6 ran 12% self-inflicted because the surface was frozen;
rounds 4 and 5 ran 86% and 79% because it was not.

What is new since round 6 — its 24 confirmed findings, all now closed. Every one
of these carries an assertion in interaction-check.mjs (253 assertions, 0
failing, up from 45 when this work started), so restating one is worthless:

- **The 720 crossing.** A ReferenceError fired on every crossing of 720 in all
  three products, so the media-query listeners had never run once. Fixed, and
  the behaviour gate now narrows ONE page across the breakpoint and widens it
  again rather than opening a fresh page per viewport.
- **The reflow.** Every configured viewport under 480 was handed a touch
  pointer, so no no-sideways-scroll assertion had ever run on a fine one. The
  dictation floor overflowed 145px at 320. Fixed; 320/390/430 now sweep on a
  mouse.
- **The 1100–1279 band.** The board was kept there and every card note was
  trimmed. The clamp floor is 1200, the board's own floor is 1199, and below it
  the list is what opens.
- **The dense fold.** A column hiding ten of thirty-two tasks now says "5 more
  below" in the Add row, and the fold rule is drawn in --ink-4 rather than in
  the same --line every card border uses.
- **The seam's landing.** The desk printed the sent sentence three times under
  two eyebrows with two open-task controls 233px apart; the margin row now
  stands down while the receipt is up, matching the phone's shape.
- **One keycap.** 11px mono at +0.04em, declared once in shell.css with one
  ring mechanism; Tasks, Notes and Timeline keep only their ink. Timeline's cap
  was bare grey text and is a key again.
- **The lead tick.** On the across axis the accented mark sat 28px left of its
  own date — 4.7 days early at 768. The desk overrides are scoped to the
  vertical axis now.
- **Undo.** The keyboard drop arms one; and the seam's crossing can be undone
  from Tasks or Timeline, not only from Notes, inside its own 30-second window.
- **Placing a moment.** On the across desk the editor opened between the horizon
  and the track and pushed all eight grab handles off screen. It opens after the
  plan there now, and keepInBand knows the sheet scroller.
- **The arrival** has a beat for the pointer, not only a focus ring for the
  keyboard. **Notes' head** stops saying "All your notes" while narrowed. **The
  filing field** has one name and one spoken grammar. **Compact** is a gated
  state on the type ramp. **The completion sentence** stops counting the same
  notes twice. **The seam's card** stops repeating its own title.

Deliberate decisions you may argue with, but must not report as oversights:
- The Notes head has no count on purpose (a tally of how far behind you are is
  the wrong loudest object on a capture surface).
- The 1120 stack overturns round 5's 1440; the reason is legibility of a row as
  a sentence, recorded.
- The timeline's gap note is EMPTY at rest by decision; "Nothing is planned
  yet." is the only sentence it carries.
- The grab handle's word is invisible by decision; the underline is the
  affordance.
- Doors that are not built say "Not here yet." in a card; the doors are kept
  because deleting an honest door is how a product starts lying about what it
  is.
- Timeline's wordmark sits lower and to the right of the other two. Two seats
  raised this in round 6 and BOTH were refuted on measurement — the claimed
  alignment contract holds at only 2 of 6 widths and every proposed fix worsens
  the phone. Raising it again needs new measurement, not a new opinion.
- The board's floor is 1199 by decision: below it the list is the view, and the
  board is still reachable there as a scroller that keeps its specimen lane.
- Two gate exclusions are REPORTED, not gated, and both are handed to you
  deliberately: **contrast** (30 open sightings, the model disagrees with the
  render) and **tracking** (24 groups where one family and size show more than
  one letterfit; the three products carry three separately ratified curves).
  Findings there are in scope and welcome.

This is the last round. If the surface is ready, say so plainly; if it is not,
name precisely what stands between it and 9.5 and what that costs.
...
guess at any other seat's opinion. Write the JSON to /home/user/app/docs/design/labs/master-suite-2026-08/panel/round-6/seat-ux.json.
