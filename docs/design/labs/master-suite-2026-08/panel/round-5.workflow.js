export const meta = {
  name: 'master-suite-panel-round-5-final',
...
const reviews = await parallel(SEATS.map((seat) => () =>
  agent(
    `You are the ${seat.name} seat on an independent design review panel. Round 5.\n\n` +
    `YOUR LENS\n${seat.lens}\n\n${BAR}\n\n` +
    NO_QUOTA,
...
log(`round 5: ${alive.length} seats, ${confirmed.length} confirmed, ${refuted.length} refuted (${Math.round(refuteRate * 100)}%)`)
