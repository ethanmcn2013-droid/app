...
## Shared timeline (all six finished)
...
| 6 | The Line | The journey drawn as a transit map: milestones are stations, parallel strands of work are coloured lines that merge at the day, and a train on each line shows where things are right now. | `9d8c8271` |

Routes: `/app/concepts/shared-timeline/1..6`. Shared-timeline concepts cover the whole screen
...
Concept 6 has a split-flap departure board ("Next stop"), "Follow a line" chips, a pannable map with "Show the whole line", stop cards with Add to my calendar, an upright map on the phone, and a Concept preview bar (wedding, launch night, river study; early days, a stop moved, a crowded month, arrived, one line only). The opening view shows today's stretch; the lines merging at the day show after "Show the whole line" or panning right.

All six Apps and tools and all six Shared timeline concepts are now built. The sprint is complete.

## Screenshots

`docs/design/concept-sprint/shots/apps/apps-{1..6}-{light,dark,phone}.jpg` and
`docs/design/concept-sprint/shots/shared-timeline/shared-timeline-{1..6}-{light,dark,phone}.jpg`, plus
