...
Integration note, 22 September: this is the original PC-08 report. The
receiving migration is now `0033_conversation_task_outcomes`, after the
January `0028`–`0031` Tasks migrations. The SQL content hash below is
unchanged; the receipt file changed to record the new ID, so its historical
receipt hash is not the receiving receipt hash. The receiving migration
ledger is authoritative. The receiving service also checks January's
Project/account deletion fences and captures a canonical sponsored-use intent
