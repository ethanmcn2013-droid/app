...
At the same 390px synthetic phone viewport, Return produced the exact textarea value “Phone return keeps composing\n”. The isolated fixture still contained exactly 20 conversation_messages, so Return did not send. This uses browser keyboard automation, not a physical keyboard/IME or human-comprehension study.


## User-requested pause, 12 September 2026

PC-10 locally accepted at application revision7669af5b73904ce0dcbd5256ce3f4bdf79bff3de. The complete current receipt, remaining work, exact fixtures and worker refs are in STATE.md; final source checks and independent red/green receipts are in evidence/reviews. Integration suite111/111, full TypeScript and scoped lint pass. Preserved review harnesses are standalone artifacts; copy .ts.txt to an isolated working test path to execute, never add it to application TypeScript inputs.

All owned previews3187–3192 stopped after process identity checks; port readback zero. No other listener was stopped. Progress heartbeat PAUSED. The retained launchers and synthetic fixtures remain under C:/Users/ethan/Documents/Codex/2026-09-12/plea/work. Final UI captures show the isolated Discussion surface at1280×720 and390×844. They do not establish authenticated App or production readiness.

The corrected browser fixture exercised three Alice operations through committed-response plus receipt-lookup outages: send, edit and tombstone. Route navigation retained each original attempt; restored receipt lookup recovered without duplicate effects. Bob's separately authored reply remained after Alice deleted the root. Direct readback: two comments, one tombstone, root revision3, four receipts. An added Dana member required current-name audience confirmation before composition resumed. Removing Alice cleared Discussion and its draft. Restoring membership and re-entering showed the authorized history with an empty composer. The fixed outer synthetic lab header remained visible during revocation; no claim is made that a parent Task title was cleared.
