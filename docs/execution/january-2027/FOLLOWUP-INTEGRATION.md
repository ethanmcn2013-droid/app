...
The principal integration worktree is feat-january-final-journeys, starting from77d. It owns shared package/CI/experience registration and final composition. The two repair lanes have distinct runtime ownership and preserve prior negative receipts. Fresh scoped review precedes release-branch integration.

The local combined candidate0cb137161fc7defa25cfb1b5a04104454df16b57 includes recipient9cbfcdd1/956a2e4f/92e455b9 and module7915f237/4691b4c6/6ba2473a. The unchanged `node experience/recipient-project-work/golden.cjs` now exits0 with all10 persisted checkpoints. It creates a private Note, extracts to Tasks, accepts an invitation, commits recipient work, synchronizes and narrowly publishes Timeline, revokes its link and returns through actual Home scope/ranking to the exact B task despite cookie A. All197 loaded source/migration/package hashes match the candidate. The original Home failure remains preserved.

This uses four disposable SQLite stores and actual application writers/queries with explicit request/framework/email adapters. It is not a continuous Next/Clerk browser or provider roundtrip. The removed-member checkpoint deletes the fixture membership row and does not test the Drive removal orchestrator. Human comprehension is unverified. Fresh independent recipient and module candidate reviews remain active; no release verdict is inferred.

A separate reproduced Timeline legacy/null-lease sync race remains open: deletion after the awaited inherited binding check and before node writes can recreate an orphan task and report success. The complete transactional eraser closes its own partial-delete boundary but does not fence that in-flight writer. Recovery owner is preparing a separate narrow sync repair after boundary/compatibility review; its red receipt remains a finding, not successful recovery evidence.
