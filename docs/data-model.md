...
| Workspace + membership | SQLite `workspaces` / `workspace_members` (prod-bound for Postgres — see decisions.md) | Per-tenant boundary. Pricing keys off this. |
| Tasks · comments · activities | SQLite `tasks` / `comments` / `activities` | The product. Everything else is supporting cast. |
| File attachments (metadata) | SQLite `attachments` | One row per upload; bytes live elsewhere (next row). |
...
| Entitlements (which tier this user has on this workspace) | SQLite `entitlements` | Highest non-expired row wins per `(userId, workspaceId)`. Resolution is in `getEffectiveTier`. |
| Comp / promo codes | SQLite `comp_codes` + `entitlements` | Mint once, redeem decrements quantity, inserts entitlement. |
| Share-link tokens + visit log | SQLite `share_links` / `share_link_visits` | Read-only public access by token. Visit counter for analytics + sparkline. |
| Notifications + prefs | SQLite `notifications` / `notification_prefs` | Anti-notification stance: only @mentions + direct blocks land here. Daily digest is the only broadcast channel. |
| Stripe webhook idempotency | SQLite `processed_webhooks` | Prevents double-grant on Stripe re-deliveries. |
| GTM roadmap state | SQLite `roadmap_items` / `blockers` / `action_items` | Operator surface at `/roadmap`. Source-of-truth markdown is in `docs/gtm-plan.md`; the parser keeps the table in sync. |
...
Today (SQLite-on-Vercel-ephemeral): not real. The committed `tasks.db` ships pre-seeded; per-instance copies in `/tmp` get rebuilt on cold start. **All user data created in production is lost on instance recycle.** This is the urgent reason `AI-data-postgres-decision` is P0.

After Postgres lands:
- Daily logical dump to Vercel Blob or S3.
- Retention 30 days.
- Restore drill quarterly (`AI-data-backups`).
- Point-in-time recovery via the provider (Neon offers it natively).
...
Today's stack is enough for an indie product at launch traffic. The two changes that matter once usage grows:

1. **Postgres swap.** Discussed in `decisions.md` § Database. Adapter goes behind the existing Drizzle calls; UI doesn't change.
2. **S3 / Vercel Blob for attachments.** Local disk doesn't survive Vercel's multi-replica model. The seam is `uploadsRoot()` + the streaming read in the API route.
