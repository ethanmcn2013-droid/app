...
The next independent pass at6d428ffa confirmed those repairs but reproduced comma-separated scalar identifiers being split by the analytics URL filter parser. The final correction validates one exact scalar before either the shared hint path or native normalizer/list parser: comma syntax, outer whitespace, control characters, blanks, arrays and overlong identifiers refuse rather than select a different project. Deliberate list filters and legacy Label compatibility remain unchanged. Sixteen Home cases plus the demo regression now pass (17 total), with typecheck and focused lint. The original failed independent repro remains history; the new scalar correction awaits a fresh reviewer result before integration.
