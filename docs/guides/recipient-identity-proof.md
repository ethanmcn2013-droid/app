...
The test uses Clerk's supported Playwright ticket helper for controlled setup and to restore the creator session after the wrong-account refusal. Those helper-issued sessions bypass credential entry and MFA. Recipient continuation is separate: the recipient follows the invitation's existing sign-in link, enters the controlled `+clerk_test` address in Clerk's visible email-code form, completes it with Clerk's fixed test OTP, and must return automatically to the exact invitation. This test mailbox flow does not deliver a real email and does not exercise password or MFA.

The journey checks a wrong-account refusal, the exact signed-out redirect intent, visible recipient email-code sign-in and automatic return, recipient acceptance into the exact project, absence of a creator-private task, completion of one assigned task, return to Home, creator browser readback of the durable done state, replay refusal, and access refusal after membership removal. Synthetic identity tests, an issued Clerk testing token, or helper-issued setup sessions do not count as journey acceptance.
