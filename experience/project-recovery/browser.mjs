...
const receipt = { status: "running", head: execFileSync("git", ["rev-parse", "HEAD"], { cwd: root, encoding: "utf8" }).trim(),
...
try {
  f = await recoveryFixture({ links: 25, publications: 25 });
  const entry = [
  ...
  for (const file of sources) receipt.sourceInputs[file] = hash((await fs.readFile(path.join(root, file), "utf8")).replaceAll("\r\n", "\n"));
  ...
  for (const viewport of [{ width: 390, height: 844 }, { width: 1440, height: 960 }]) for (const theme of ["light", "dark"]) {
    ...
    assert.equal(await page.getByRole("button", { name: "Revoke link", exact: true }).count(), 20);
    assert.equal(await page.getByRole("button", { name: "Revoke publication links", exact: true }).count(), 20);
  ...
  await page.getByRole("button", { name: "Revoke link", exact: true }).last().click();
  ...
  assert.equal(await member.getByRole("button", { name: "Revoke link", exact: true }).count(), 20);
  ...
  assert.equal(await timelineOnly.getByRole("button", { name: "Revoke publication links", exact: true }).count(), 20);
  await timelineOnly.getByRole("button", { name: "Revoke publication links", exact: true }).first().click();
  await timelineOnly.getByText("Access withdrawn. The controls are refreshing.", { exact: true }).waitFor();
  assert.equal((await f.timeline.select().from(f.timelineSchema.audienceShares).where(eq(f.timelineSchema.audienceShares.id, "share-24")))[0].state, "revoked");
  ...
  assert.equal(await unavailable.getByRole("button", { name: "Revoke link", exact: true }).count(), 0);
