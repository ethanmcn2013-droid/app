...
the four behavioral SQLite scenarios. Selector coverage is in the already
...
the committed App migrations, under ignored `experience/output/recipient-project-work/stores`, and closes the connection. Files
...
The actual `/app/task/[id]` page regression also follows the persisted task's
own project into the existing authorization and scoped detail reader. It catches
the production mapper mismatch that the older injected-resolver test missed:
`getTaskById()` returns a client Task without `workspaceId`. The route now reads
only the stored project ID before proving membership. Foreign, missing, removed
and account-switched callers issue no content query; authorized archived tasks
retain their read-only view. GET does not write an active-project cookie.
