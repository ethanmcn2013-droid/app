...
## 2026-09-08 · T·155 · ships · the front door opens on all three rooms

**The homepage hero is now the three product cards from the 2 September
front-door directions — a note, the task it becomes, and the date that goes
live on the couple's timeline — under the locked suite headline. The
Tasks-only departure board and the domain-toggled cinematic demo are off
the page.**

The cards come from the founder's pick of the three landing directions
(`remote-redesign/work/2026-09-02-landing-directions`, A · Floor and sheet,
shipped to signalstudio.ie via studio #174). They are ported, not pasted:
`src/components/marketing/hero-relay.tsx` rebuilds the relay on the
design-system tokens — one easing, four durations, two shadows, the
system radii — and speaks this product's lane vocabulary, so the task
goes from To do to Moving rather than to "In progress". The sequence is the
direction's: the note arrives and the approved sentence is swept, the line
leaves the note and lands as a card with an owner and a day, the card
starts moving, a confirmed date leaves it for the timeline, 79 counts up,
the track draws, the milestone goes live. Six seconds, once, as the stage
arrives; "Play again" appears when it has finished and is `hidden` until
then, so it is never a focus stop for an invisible control. Settled panes
sit at three depths under the pointer on hover devices.

Three things the port does that the direction did not have to. The server
renders the settled state, so no crawler, no-JavaScript visitor or
reduced-motion reader ever meets an empty stage; the scene takes the stage
back to its start in a layout effect, before the first paint. Reduced
motion is honoured live — if the preference flips mid-sequence the stage
jumps to the end, which is also what the browser evidence run does 300ms
after load. And the timeline's date labels are governed by a container
query on the card, so the two middle labels and "Today" step out when the
card is narrower than 430px instead of colliding on a phone.

The copy is the locked headline, "Project management for people not in
tech.", with "not" in indigo at the founder's request, and the direction's
lede. The primary action is the waitlist, as the header and the closing
CTA already were; the secondary scrolls to the card anatomy. `id="demo"`
stays on the stage so the footer's Demo link and the suite deep-links keep
landing.

Removed: `TasksHeroTicker` (T·79's departure board, in history at
`71a72c4` if it is ever wanted back), `DomainToggle`, and the `.demo-fit`
phone scaling that existed only for the cinematic demo. The showcase
components stay: the inbox, My Week, the palette and the detail panel
still import from them; only `cinematic-demo.tsx` is now unmounted.

Gates: the root fixture now asserts the new headline; the root page's
materiality hash was refreshed through `experience:fixtures:write`; and
because the fixture manifest is part of the evidence contract, all twenty
materiality receipts were re-issued against a fresh 132/132 browser run on
the production build in demo mode. Eight receipts from 13 August that the
17 August reviews had already superseded, but which were still on disk and
pinned to the old manifest, are removed with them. Typecheck, lint, the first-contact
language check, the waitlist contract, the tap-target gate and the route
manifest are clean. The signalstudio.ie hero lives in the `studio` repo,
which this session could not reach; the component ports there as one file.
