...
## 2026-09-05 · T·110 · tightens · the last round, and a primary that had never once looked disabled

**Every blocking and every misleading finding the panel has ever confirmed against the Signal Studio master is now closed, and the worst of them was a button that reported itself enabled, looked enabled, and did nothing.** Round 6's seventeen standing findings are closed. Round 7 — the last of the engagement — ran seven blind seats and thirty-five fresh refuters against a frozen build and found seven more of the kind that stop a person or mislead one. All seven are closed. Twenty-three findings stand, and not one of them is blocking or misleading.

The seam's Send was dead and swapped. Clearing the wording field called `toggleAttribute`, which writes the empty string, and `aria-disabled=""` is an invalid token a browser resolves to false — so the accessibility tree reported an ordinary enabled button while click and Enter did nothing, and every rule written to grey it out stopped matching. Five clearing routes at four widths. First paint was always correct, which is why six rounds of static sweeps never saw it. Worse: swapping the button's own label rewrapped the footer and moved "Never mind" into the rectangle Send had occupied a frame earlier, so reaching back for Send threw the peel away. And the adversarial reader proved something neither the seat nor the fix had reached — the primary has never had a disabled appearance in any state, so repairing the attribute alone would have shipped a lie with valid ARIA.

Three more of that weight. In the List — the default view below 1200 and the only view a phone gets — a finished row and an unfinished row differed in **zero** computed properties across the tick, its check and both pseudo-elements; under forced colours the list painted a check on all thirteen rows, eight of them not done. The keyboard carry had five exits and only one recorded the move, so Enter, Tab, a rail switch and a pointer press each committed a lane change the undo then denied — and Tab moved focus into the carried card's own tick, where Space finished the task while it was still in hand. The More door announced "More, closed." as it opened, and told the truth only to people who ask for reduced motion, because that path runs its mutation a frame earlier.

Four of the seven were invisible to reading and only appeared under driving. Three sat in code written the same day. The behaviour gate went 45 → 298 assertions across the engagement, 187 → 298 in these two rounds alone, and every fix here was written as a failing assertion first. Two of those assertions passed vacuously when first written and were repaired before the fixes landed — the thirteenth time this lab has recorded absence reading as a pass, and the first time it was caught in the same hour it was created.
