...
>>>>>>> d64c1f6f (Keep Google Drive disconnect requests durable until provider confirmation)
...
This entry records the candidate, not a completed production deployment. Linux CI passed the original patch's type, test, build and migration gates; the final revision must pass the full receiving checks and rendered preview verification before release. Remaining dependency findings and the Windows SQLite cleanup failures are recorded in `docs/security/nextjs-critical-update-2026-09-22.md`.
...
default-gate contract test holds the committed artwork, every ICO frame and
all three icon routes to that one source.**

T·153 fixed the drift once; this fixes the mechanism. `favicon.ico` is a
static file Next serves verbatim at `/favicon.ico`, independent of the
`/icon` routes, so any future artwork change can update the routes and
leave the fallback behind — which is exactly what had happened before
T·153. Two things now prevent it. The generator renders the real
`SuiteMark` component through the same `ImageResponse` path as `/icon`
rather than re-implementing the circle, so the ICO owns no brand constants
of its own. And `scripts/brand/favicon-contract.test.mjs` runs first in
`pnpm test`, asserting the committed `favicon.ico` byte-matches a fresh
render, that every frame is a well-formed PNG at its declared size, and
that `/icon`, `/apple-icon` and `/icon1` still render the same artwork.

The mark itself is unchanged from T·153 — ink field, single indigo dot,
40% of the canvas — and every regenerated frame was verified to carry
`#17171a` and `#6366f1`. Regenerate with `pnpm brand:icons` after an
intentional artwork change, then update the seal in the contract test.

An artwork seal pins `src/lib/brand/suite-mark.tsx` by SHA-256, so a mark
edit fails the gate until someone has looked at the rendered icon and
re-pinned it deliberately. That is the point: the seal is a checkpoint,
not a lock.
