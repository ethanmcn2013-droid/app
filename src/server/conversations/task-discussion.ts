...
export function createTaskDiscussionService(adapter: ConversationDatabaseAdapter) {
  ...
  async function openTaskDiscussion(
    ...
    return inTransaction("write", async (executor) => {
      // Opening can lazily create discussion state, so prove the exact Task
      // and hold the same deletion fences before that first insert.
      const destination = await executor.execute({
        sql: `SELECT t.workspace_id FROM tasks t
          JOIN workspace_members wm ON wm.workspace_id=t.workspace_id AND wm.user_id=?
          JOIN users actor ON actor.id=wm.user_id
          WHERE t.id=?`,
        args: [input.actorId, input.taskId],
      });
      if (!destination.rows[0] || !await conversationWriteFencesClear(
        executor, input.actorId, text(destination.rows[0].workspace_id),
  ...
  async function sendComment(args: {
    ...
    return inTransaction("write", async (executor) => {
      ...
      if (!await conversationWriteFencesClear(executor, actorId, authorized.value.projectId)) return failure("unavailable");
  ...
  async function mutateComment(
    ...
    return inTransaction("write", async (executor) => {
      ...
      if (!gate.ok) return gate;
      if (!await conversationWriteFencesClear(executor, args.actorId, authorized.value.projectId)) return failure("unavailable");
      if (!await validateMentionMembers(executor, authorized.value.projectId, mentions)) return failure("invalid_input");
  ...
  async function observeRange(input: ActorTask & {
    ...
    return inTransaction("write", async (executor) => {
      const authorized = await authorizeTask(executor, input);
      if (!authorized.ok) return authorized;
      if (!await conversationWriteFencesClear(executor, input.actorId, authorized.value.projectId)) return failure("unavailable");
