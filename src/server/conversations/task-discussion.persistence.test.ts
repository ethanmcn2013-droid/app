...
const migration31 = "0035_task_discussion.sql";
...
test("Task Discussion writes honor Project and account deletion fences without hiding committed history", async () => {
  const f = await freshDatabase();
  try {
    const service = serviceFor(f.client);
    const opened = await service.openTaskDiscussion({ actorId: "synthetic_bob", taskId: "task_a" });
    if (!opened.ok) assert.fail("discussion setup failed");
    assert.equal((await service.sendComment({ actorId: "synthetic_bob", input: sendInput(opened.value.audienceEpoch, "before_task_delete_fence") })).ok, true);
    await f.client.execute({
      sql: `INSERT INTO project_drive_operations(id,workspace_id,operation_kind,status,dedupe_key)
        VALUES ('delete_discussion_project',?,'project_delete','pending',?)`,
      args: [projectA, "c".repeat(64)],
    });
    assert.deepEqual(await service.sendComment({ actorId: "synthetic_bob", input: sendInput(opened.value.audienceEpoch, "during_task_delete_fence") }), { ok: false, code: "unavailable" });
    assert.equal((await service.getHistory({ actorId: "synthetic_bob", taskId: "task_a", afterChangeSeq: 0 })).ok, true);
    await f.client.execute("DELETE FROM project_drive_operations WHERE id='delete_discussion_project'");
    ...
    assert.deepEqual(await service.sendComment({ actorId: "synthetic_bob", input: sendInput(opened.value.audienceEpoch, "owner_task_erasure_fence") }), { ok: false, code: "unavailable" });
    ...
    assert.deepEqual(await service.sendComment({ actorId: "synthetic_bob", input: sendInput(opened.value.audienceEpoch, "actor_task_erasure_fence") }), { ok: false, code: "unavailable" });
    assert.equal(Number((await f.client.execute("SELECT COUNT(*) AS n FROM comments WHERE task_id='task_a'")).rows[0].n), 1);
  } finally { f.client.close(); }
});

test("0035 preserves canonical legacy ids and quarantines only unprovable tenant rows", async () => {
  const fixtureRoot = process.env.PC10_WORK_DIR ? resolve(process.env.PC10_WORK_DIR) : tmpdir();
  await mkdir(fixtureRoot, { recursive: true });
  ...
  try {
    await client.execute("PRAGMA foreign_keys=OFF");
    await applyMigrations(client, "0034_project_direct_messages.sql");
