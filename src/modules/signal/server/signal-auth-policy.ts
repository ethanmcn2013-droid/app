/**
 * Pure auth-policy predicate for the Signal module — no server-only import
 * so the fail-closed invariant is unit-testable outside the Next server context.
 *
 * Mirrors src/modules/timeline/server/timeline-auth-policy.ts exactly.
 * A cross-module import of the Timeline version is intentionally NOT used:
 * check-module-boundaries.mjs (rule 1) would flag src/modules/signal/…
 * importing from src/modules/timeline/…. The predicate is six lines and pure;
