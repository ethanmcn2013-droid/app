...
function ProjectConversationSession({ actorId, project, projects, fixtureActor, directId, rootId = null, onCloseThread, draftCache, outgoingCache, scrollCache, view, onToggleView }: Readonly<{ actorId: string; project: ProjectOption; projects: readonly ProjectOption[]; fixtureActor?: string; directId?: string; rootId?: string | null; onCloseThread?: () => void; draftCache: DraftCache; outgoingCache: OutgoingCache; scrollCache: ScrollCache; view: "full" | "context"; onToggleView: () => void }>) {
  ...
  async function send() {
    ...
    if (!scope || (scope.kind === "dm" && !scope.canWrite) || !audienceReady || state.audienceEpoch === null || state.reviewedAudienceEpoch !== state.audienceEpoch || !validMessageBody(state.draft) || sendsOff || scope.lifecycle === "archived") return;
  ...
  return <main className={styles.main} data-thread={rootId ? "true" : undefined}>
    ...
    <AudienceHeader onShowDetails={() => setShowDetails((value) => !value)} description={audienceDescription} onToggleRelated={() => {}} relatedOpen={false} showRelatedWork={false} status={scope.lifecycle === "archived" ? "archived" : scope.kind === "dm" && !scope.canWrite ? scope.pairState === "blocked" ? "blocked" : "pending" : "active"} title={title} />
    ...
    <div className={styles.viewBar}><button onClick={onToggleView} type="button">{view === "full" ? "Context panel" : "Full view"}</button><span>{scope.kind === "dm" && !scope.canRead ? "Participation required" : state.status === "offline" ? "Offline · retrying" : state.status === "ready" && !catchingUp ? "History up to date" : "Updating history"}</span></div>
    ...
    <div aria-label="Message reading area" role="region" tabIndex={0} className={styles.feedScroller} onScroll={() => rememberScroll(scope)} ref={feedRef}><ol aria-label="Conversation history" className={styles.feed}>
      ...
      {state.messages.map((message) => <LiveMessage onReply={!rootId ? () => { replyTriggerRef.current = document.activeElement instanceof HTMLElement ? document.activeElement : null; setOpenedRoot(message.id); } : undefined} actorId={actorId} editing={editing} key={message.id} members={members} message={message} taskCreated={Boolean(createdTasks[message.id])} onCreateTask={audienceReady && !audienceNeedsReview && !sendsOff && scope.lifecycle === "active" && (scope.kind !== "dm" || scope.canWrite) ? () => { taskTriggerRef.current = document.activeElement instanceof HTMLElement ? document.activeElement : null; setTaskSource({ sourceProjectId: project.id, conversationId: scope.conversationId, messageId: message.id, expectedRevision: message.revision, expectedAudienceEpoch: state.audienceEpoch! }); } : undefined} onCancelEdit={() => setEditing(null)} onDelete={() => void mutate("tombstone", message)} onEdit={(body) => void mutate("edit", message, body)} onStartEdit={() => message.body && setEditing({ id: message.id, body: message.body, revision: message.revision })} setEditing={setEditing} />)}
    ...
    {openedRoot && !rootId ? <section className={styles.threadPanel} aria-label="Message replies"><ProjectConversationSession key={openedRoot} rootId={openedRoot} onCloseThread={() => { setOpenedRoot(null); requestAnimationFrame(() => replyTriggerRef.current?.focus()); }} directId={directId} actorId={actorId} project={project} projects={projects} fixtureActor={fixtureActor} draftCache={draftCache} outgoingCache={outgoingCache} scrollCache={scrollCache} view="context" onToggleView={() => setOpenedRoot(null)} /></section> : null}
...
function LiveMessage({ onReply, actorId, message, members, editing, setEditing, onStartEdit, onCancelEdit, onEdit, onDelete, onCreateTask, taskCreated }: Readonly<{ onReply?: () => void; onCreateTask?: () => void; taskCreated: boolean; actorId: string; message: MessageRecord & { replyCount?: number }; members: Audience["members"]; editing: { id: string; body: string; revision: number } | null; setEditing: (value: { id: string; body: string; revision: number } | null) => void; onStartEdit: () => void; onCancelEdit: () => void; onEdit: (body: string) => void; onDelete: () => void }>) {
