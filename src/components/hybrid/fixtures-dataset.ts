// The frozen design-lab dataset: 48 seeded tasks, their descriptions and
// the shapes built from them. Nothing in the product imports this — the
// production board hydrates from the server — so it lives apart from
// fixtures.ts, whose registries the client genuinely needs. Keeping the
// dataset out of that module keeps ~24 KB of demo data out of every
// client bundle. The data itself is byte-identical; fixtures.test.ts
// still hashes LAB_TASKS against FIXTURE_SHA256.
...
import { LAB_PEOPLE } from "./fixtures";
type Seed = {
  id: string;
  title: string;
  status: TaskStatus;
