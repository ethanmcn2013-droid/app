...
export type TasksFlight = {
  /** Call before the state change, naming the card that is about to move.
   *  `from` overrides where the card starts, e.g. a dropped drag ghost. */
  arm: (id: string, from?: DOMRect) => void;
};

const REDUCED = () =>
  typeof matchMedia === "function" && matchMedia("(prefers-reduced-motion: reduce)").matches;

export function useTasksFlight(
  ...
  const arm = (id: string, from?: DOMRect) => {
