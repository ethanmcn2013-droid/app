...
export function useTasksUndo(onUndo: (act: TasksAct) => void) {
  ...
  const [history, setHistory] = useState<TasksAct[]>([]);
  const stack = useRef<TasksAct[]>([]);
  const [showing, setShowing] = useState<TasksAct | null>(null);
  const timer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const undoing = useRef(false);

  const write = useCallback((next: TasksAct[]) => {
    stack.current = next;
    setHistory(next);
  }, []);
  ...
  const arm = useCallback((act: TasksAct) => {
  ...
  const forget = useCallback((id: string, kind?: TasksAct["kind"]) => {
